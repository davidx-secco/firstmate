...
test_composer_state_cursor_bare_placeholder_is_empty() {
  ...
  # Cursor's real idle composer draws its follow-up placeholder as SGR-2 dim ghost
  # text after the bare `â` agent glyph. The styled capture is what proves it is a
  # placeholder rather than typed input, which is why the fixture carries the
  # styling: bright text of the same string is a genuine draft and stays pending
  # (test_composer_state_styled_placeholder_draft_is_pending).
  printf '  Cursor Agent\n\n\xe2\x86\x92 \x1b[0m\x1b[2mAdd a follow-up\x1b[0m\n\n  Ask (shift+tab to cycle)\n' > "$resp/1.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_composer_state default:w1:p2' "$ROOT" )
  [ "$out" = empty ] || fail "the dim Cursor follow-up placeholder should read empty, got '$out'"
  pass "fm_backend_herdr_composer_state: Cursor Agent's dim idle placeholder reads empty"
}
