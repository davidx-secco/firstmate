...
test_cursor_busy_signature_is_in_shared_default() {
  ...
  # Cursor crewmate as idle.
  printf '%s\n' '→ Add a follow-up    ctrl+c to stop' \
    | fm_busy_lines_match cursor \
    || fail "Cursor Agent's stop hint must match its harness-scoped signature"
  printf '%s\n' 'Ctrl+c:cancel' \
    | fm_busy_lines_match cursor \
    && fail "Cursor must ignore Grok's cancel footer"
  printf '%s\n' 'esc to interrupt' \
    | fm_busy_lines_match cursor \
    && fail "Cursor must ignore Claude's interrupt footer"
  pass "Cursor Agent's busy hint is in the shared default and its own scoped signature"
}
