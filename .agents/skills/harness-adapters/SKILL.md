...
The `config/crew-harness` step skips a crewmate/scout-only harness (cursor) rather than inheriting it, with a loud reason on stderr; `docs/configuration.md` owns the details.
...
If the captain asks for a new harness, propose verifying it first: spawn a trivial supervised task using `fm-spawn`'s raw-launch-command escape hatch, confirm every fact empirically, then record the mechanics in `fm-spawn`, its semantic busy source and trust gate in `bin/fm-busy-lib.sh`, any needed `FM_COMPOSER_IDLE_RE` empty-composer override plus any novel bare agent prompt glyph in `bin/fm-composer-lib.sh`'s shared composer classifier (the one fleet-wide owner of the empty/dead-shell/pending decision, so a new harness's own idle composer is not misread as a dead shell), the delivery-only rendered busy footer in `bin/fm-composer-lib.sh`'s `FM_COMPOSER_BUSY_REGEX_DEFAULT` for the UNSCOPED fallback (one place fleet-wide - every backend and `fm-tmux-lib.sh` alias it, and the per-adapter `FM_*` names stay the operator override surface) plus that same footer again as a harness-scoped `FM_TMUX_<HARNESS>_BUSY_REGEX_DEFAULT` and its `fm_busy_lines_match` case in `bin/fm-tmux-lib.sh` (an unregistered harness gets an empty scoped regex and is therefore never matched by the submit-acknowledgement and away-mode delivery guards), the tmux agent-process liveness classification in `bin/backends/tmux.sh` when the harness can launch a secondmate, and the verified knowledge here.
...
`bin/fm-harness.sh secondmate` resolves the secondmate-launch harness through the chain `config/secondmate-harness` -> `config/crew-harness` -> own, so an unset `config/secondmate-harness` matches the crew harness unless that harness is crewmate/scout-only (cursor), which is skipped to own/`claude` with a `crew-only-harness-skip:` warning on stderr; `docs/configuration.md` owns the details.
Bootstrap's secondmate liveness sweep republishes that reason as a `SECONDMATE_LIVENESS: ... respawned ... with a substituted harness` fact, so an unattended respawn on a substituted harness is never silent.
...
| cursor | `--model <model-id>` | mapped to `cursor-grok-4.5-low`, `cursor-grok-4.5-medium`, or `cursor-grok-4.5-high` | Verified 2026-07-23 on Cursor Agent 2026.07.20-8cc9c0b. Cursor has no standalone effort flag. Effort maps to a concrete model only when no concrete model is selected (absent, or the flag-less `default` sentinel): `low` and `medium` map directly; `high`, `xhigh`, and `max` map to the highest listed Cursor Grok 4.5 model, `cursor-grok-4.5-high`. Any concrete model wins unchanged. |
...
| Turn-end notification | No per-turn hook is verified. Cursor has no entry in `fm_busy_sources_for_harness` and no rendered-tail arm, so a cursor task classifies `unknown missing` - except on herdr, whose harness-agnostic native arm can still report `busy herdr-native`; `docs/cursor-agent-harness.md` "Supervision signals" owns that detail. Treat an `unknown` verdict as "not provably working", not as idle. The rendered `ctrl+c to stop` hint feeds the submit-acknowledgement and away-mode delivery guards plus the composer classifiers, not worker state. |
