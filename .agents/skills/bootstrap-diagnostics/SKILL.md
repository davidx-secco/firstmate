...
- `SECONDMATE_LIVENESS: secondmate <id>: respawned after <cause> with a substituted harness: <reason>` - the respawn SUCCEEDED, but `config/crew-harness` names a crewmate/scout-only harness (cursor) that cannot back a secondmate, so the secondmate now runs on the substituted harness named in `<reason>`. Pin `config/secondmate-harness` if a different harness is wanted; see `docs/configuration.md`.
