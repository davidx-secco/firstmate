...
# composer owner sourced above (FM_COMPOSER_{BUSY,IDLE}_REGEX_DEFAULT in
# bin/fm-composer-lib.sh), so the fleet-wide generic signature is still added in
# exactly one place and the tmux-facing names stay stable for callers and tests.
# The per-harness signatures below are the SCOPED set: deliberately narrower, so
# one harness never borrows another harness's footer.
