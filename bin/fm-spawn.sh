...
#   (claude|codex|opencode|pi|pi-signed|grok|kimi|cursor) overrides it for this spawn.
#   Cursor is crewmate/scout-only and is refused for --secondmate; every other
#   adapter name applies to either kind. A non-flag string containing
...
sleep 0.3
# Export the selected account's credential into the worker's own shell, so every
# gh, gh-axi, and pipeline call in this lane inherits the right identity while
# global gh state stays untouched and concurrent lanes on other accounts are
# unaffected. The credential is materialized by the pane's own shell rather than
# typed into it, so it never reaches the pane's scrollback; a token that cannot
# be materialized leaves GH_TOKEN unusable rather than silently falling back to
# the active account (bin/fm-gh-account.sh's "env" contract).
