...
# bounds that home's startup-memory curation, primary config/gh-accounts gives
# that home's workers the same owner-to-GitHub-account mapping, and primary
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend gh-accounts herdr-presentation-spaces startup-memory-budget trace-context}"
