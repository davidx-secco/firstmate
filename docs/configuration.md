...
That fallback never inherits a crewmate/scout-only harness: when `config/crew-harness` is `muse`, secondmate resolution skips it with a warning on stderr and resolves to the primary's own harness, falling back to `claude`, so a secondmate respawn is not blocked by the crew setting.
