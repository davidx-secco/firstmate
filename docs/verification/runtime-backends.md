...
Cursor Agent remained a generic `node` process and is intentionally inconclusive; a generic interpreter is `unknown`, never confidently dead, so an ambiguous read cannot license a duplicate process.
Cursor is now resolved from its own executable name and install tree rather than that generic `node` identity, so the gap above does not weaken secondmate liveness; the "Cursor Agent CLI" section below owns the current evidence, and [`../cursor-agent-harness.md`](../cursor-agent-harness.md) keeps the earlier crewmate-era capture.
